...
class Game {
  ...
  start() {
    if(this.state == 0 && this.mode == "siege") {
      this.state = 1;
      setTimeout(() => {
        this.startTimer();
        this.mapIndex = 1;
    ...
    if(this.mode == "tdm" && this.state == 0 && this.gameOn && this.players >= 1) {
      this.state = 1;
      this.startTimer();
      console.log("TDM time, GAME ON");
    }
  }
  check() {
    ...
    if(this.state == 0 && this.mode == "tdm" && this.players >= 1) {
      this.start();
    }
  }
  playerJoin(p) {
  ...
  update() {
    if(this.state == 2) {
      ...
      for(let i in this.playerList) {
        ...
        if(p.joined) {
          this.lb.push({name: p.name, score: p.score, team: p.team});
        }
      }
  ...
  reset() {
    ...
    once = 0;
    mode = Math.round(Math.random() * 1) + 0;
    this.mode = gameKeys[mode];
  }
}
...
let bInit = [];
let id = Math.random();
let gameKeys = ["ffa", "tdm"];
let mode = Math.floor(Math.random() * 2) + 0;
let game = new Game("tdm", id);
let gameRedis = JSON.stringify({room: game.room, numClients: game.players, port: 2000});
client.set("0", gameRedis, redis.print);
...
io.on("connection", (socket) => {
  ...
  game.players++;
  gameRedis = JSON.stringify({room: game.room, numClients: game.players, port: 2000});
  client.set("0", gameRedis, redis.print);

  socket.emit("gameMode", game.mode)
  ...
  socket.on("init", (data) => {
    if(game.playerList[socket.id] !== undefined) {
      ...
      if(game.players % 2 === 0 && game.mode !== "ffa") {
        let p = genPos();
        game.addPlayer(new Player(data.name, socket, [gunList[data.wep.p], gunList[data.wep.s]], 0, p, id), socket);
      } else if(game.mode !== "ffa"){
        let p = genPos();
        game.addPlayer(new Player(data.name, socket, [gunList[data.wep.p], gunList[data.wep.s]], 1, p, id), socket);
      } else if (game.mode == "ffa") {
        let t = Math.random();
        let p = genPos();
  ...
  if(game.timer >= 0 && once == 0) {
    ...
    for(let i in game.playerList) {
      ...
      pkgp.push({
        ...
        hp: p.health,
        score: p.score,
        team: p.team % 1 == 0 || p.team == 0 ? p.team : 0,
